...
export const bindEvents = () => {
  ...
  $('.card-search-input').onkeydown = handlers.searchCards;
  // 보드 추가
  ...
  $('.board-creation-modal-background').onclick =
    handlers.closeBoardCreationModal;
  // 보드 추가 인풋
...
const handlers = {
  async showBoards() {
    ...
    const recentBoard = allBoards.sort(
      (board1, board2) => board1.recent_open < board2.recent_open,
    )[0];
    ...
    allBoards.forEach((board) => {
      ...
      if (board.is_starred)
        starredBoardsElm += `<div class="board-result-item" data-board-id="${board.id}"><span class="bg-squre modal-icon" style="background-image:url(${board.background_image})"></span><span class="board-li-elms">${board.board_name}</span></div>`;
    });
  ...
  async toggleStar({ target }) {
    ...
    axios.patch(`/boards/${state.currentBoard.id}/star`, {
  ...
  async checkIfblank(e) {
    ...
    if (e.keyCode === 13) await handlers.createNewBoard();
  },
  ...
  closeBoardCreationModal({ target }) {
    if (
      !(
        target.matches('.board-creation-modal-background') ||
        target.matches('.cancel-modal')
      )
    )
      return;
  ...
  async endSettingBoardName(e) {
    ...
    await axios.patch(`/boards/${state.defaultBoardId}/board_name`, {
  ...
  mainTab(e) {
    ...
    document
      .querySelector('.tab-wrapper-active')
      .classList.remove('tab-wrapper-active');
  ...
  setBgImage({ target }) {
    ...
    document.querySelector(
      '.bg-squre',
    ).style.backgroundImage = `url(${target.dataset.src})`;
    axios.patch(`/boards/${state.currentBoard.id}`, {
