...
const dataMethod = {
  // 초기데이타 get
  async getMainData() {
    const responseLists = await axios.get(
      `/boards/${state.currentBoard.id}/lists`,
    );
  ...
  async removeList(listId) {
    const response = await axios.delete(
      `boards/${state.currentBoard.id}/lists/${listId}`,
      { id: listId },
    );
  ...
  async patchListName(listName, listId) {
    const response = await axios.patch(
      `boards/${state.currentBoard.id}/lists/${listId}/list_name`,
      { name: listName },
    );
  ...
  async addCard(listId, card) {
    console.log(listId);
    const response = await axios.post(
      `/boards/${state.currentBoard.id}/lists/${listId}/cards`,
      card,
    );
    const listsData = await response.data;
    ...
    refreshActivityLog({
      name: state.user.full_name,
      act: `${card.card_name} 카드를 ${
        state.currentBoard.lists.find((list) => list.id === listId).name
      } 리스트에 추가 `,
      local: `card-${card.id}`,
    });
  },
  ...
  async removeCard(listId, cardId) {
    const response = await axios.delete(
      `/boards/${state.currentBoard.id}/lists/${listId}/cards/${cardId}`,
    );
  ...
  async patchCardName(listId, cardId, cardName) {
    await axios.patch(
      `boards/${state.currentBoard.id}/lists/${listId}/cards/${cardId}/card_name`,
      { card_name: cardName },
    );
    refreshActivityLog({
      name: state.user.full_name,
  ...
  async addDragCard(listId, card) {
    const responseCards = await axios.post(
      `/boards/${state.currentBoard.id}/lists/${listId}/cards/drag`,
      card,
    );
    ...
    [...cardBox.parentNode.children].forEach((cardNode) => {
      _cards = [
        ..._cards,
        cardsAfterDrag.find((_card) => _card.id === +cardNode.id.split('-')[1]),
      ];
    });
    console.log(_cards);
  ...
  async removeDragCard(listId, cardId) {
    const response = await axios.delete(
      `/boards/${state.currentBoard.id}/lists/${listId}/cards/${cardId}/drag`,
    );
  ...
  async afterListDragChangeData(_lists) {
    ...
    const response = await axios.post(
      `/boards/${state.currentBoard.id}/lists/all`,
      _lists,
    );
    const listsData = await response.data;
    lists = await listsData;
  ...
  async patchDescriptionContent(listId, cardId, content) {
    await axios.patch(
      `boards/${state.currentBoard.id}/lists/${listId}/cards/${cardId}/description`,
      { description: content },
    );
    refreshActivityLog({
      name: state.user.full_name,
...
const mainEventHandlers = {
  ...
  // 리스트 추가 제거
  addList(listName) {
    const generatedListId = () =>
      lists.length ? Math.max(...lists.map((list) => list.id)) + 1 : 1;
  ...
  addCard(cardName, _listId) {
    ...
    const generatedCardId = () =>
      cardsAll.length ? Math.max(...cardsAll.map((card) => card.id)) + 1 : 1;
  ...
  dragCard(e) {
    ...
    const listId = cardShadow.parentNode.parentNode.parentNode.parentNode.id.split(
      '-',
    )[1];
  ...
  dragEnterCard(e) {
    if (dragTarget === 'card') {
      ...
      if (e.target.className === 'card-add-box')
        e.target.previousElementSibling.appendChild(cardBox);
      if (e.target.className === 'card-content')
        e.target.parentNode.parentNode.parentNode.insertBefore(
          cardBox,
          e.target.parentNode.parentNode,
        );
      if (e.target.className === 'list-open-add-mod-btn')
        e.target.parentNode.parentNode.appendChild(cardBox);
    }
    if (dragTarget === 'list') {
      ...
      if (e.target.className === 'list-wrapper')
        e.target.parentNode.insertBefore(listBox, e.target);
      if (e.target.className === 'list-add-wrapper')
        e.target.previousElementSibling.appendChild(listBox);
      if (e.target.className === 'main')
        e.target.firstElementChild.appendChild(listBox);
      if (e.target.className === 'card-box')
        e.target.parentNode.parentNode.parentNode.parentNode.parentNode.insertBefore(
          listBox,
          e.target.parentNode.parentNode.parentNode.parentNode,
        );
      if (e.target.className === 'list-header')
        e.target.parentNode.parentNode.parentNode.parentNode.insertBefore(
          listBox,
      ...
      if (e.target.className === 'card-content')
        e.target.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.insertBefore(
          listBox,
          e.target.parentNode.parentNode.parentNode.parentNode.parentNode
            .parentNode,
        );
      if (e.target.className === 'card-shadow')
        e.target.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode.insertBefore(
          listBox,
          e.target.parentNode.parentNode.parentNode.parentNode.parentNode,
        );
      if (e.target.className === 'list-add-box')
        e.target.parentNode.previousElementSibling.appendChild(listBox);
    }
  },
  dropCard(e) {
    if (dragTarget === 'card') {
    ...
    if (dragTarget === 'list') {
      ...
      [...listShadow.parentNode.parentNode.children].forEach((listNode) => {
        _lists = [
          ...
          lists.find(
            (_list) =>
              _list.id === +listNode.firstElementChild.id.split('-')[1],
          ),
        ];
      });
...
const mainEventBindings = () => {
  document.querySelector('main').onclick = ({ target }) => {
    // console.log(target);
    if (target.matches('.add-mod-close-btn'))
      mainEventHandlers.closeAddMod(target);
    else if (target.matches('.card-add-mod-close-btn'))
      mainEventHandlers.closeAddMod(target);
    else if (target.matches('.open-add-mod-btn'))
      mainEventHandlers.openAddMod(target);
    else if (target.matches('.list-open-add-mod-btn'))
      mainEventHandlers.openAddMod(target);
    else if (target.matches('.remove-list-btn'))
      mainEventHandlers.removeList(target.parentNode.parentNode.parentNode.id);
    else if (target.matches('.card-add-btn')) {
      if (!target.parentNode.previousElementSibling.value) return;
      mainEventHandlers.addCard(
        target.parentNode.previousElementSibling.value,
        target.parentNode.parentNode.parentNode.parentNode.parentNode.id,
      );
      target.parentNode.previousElementSibling.value = '';
    } else if (target.matches('.card-remove-btn'))
      mainEventHandlers.removeCard(
        target.parentNode.parentNode.parentNode.id,
        target.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode
          .id,
      );
    else if (target.matches('.list-header-name'))
      mainEventHandlers.openListNameInput(target);
    else if (target.matches('.card'))
      mainEventHandlers.cardModalOpen(
        target.parentNode.parentNode.parentNode.id,
        target.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode
          .id,
      );
  ...
  $modalWrapper.onclick = ({ target }) => {
    if (target.matches('.modal-close-btn'))
      mainEventHandlers.cardModalClose(target);
    if (target.matches('.modal-card-head'))
      mainEventHandlers.openListNameInput(target);
    if (target.matches('.modal-description-content'))
      mainEventHandlers.openAddMod(target);
    if (target.matches('.description-edit-close-btn'))
      mainEventHandlers.closeAddMod(target);
  };
  ...
  $mainWrapper.onkeyup = (e) => {
    ...
    const listName = e.target.value.trim();
    if (e.keyCode !== 13 || listName === '') return;
    mainEventHandlers.patchListName(
      listName,
  ...
  $modalWrapper.onkeyup = (e) => {
    if (e.target.matches('.modal-card-name-input')) {
      const cardName = e.target.value.trim();
      if (e.keyCode !== 13 || cardName === '') return;
      mainEventHandlers.patchCardName(
    ...
    if (e.target.matches('.modal-description-input')) {
      ...
      mainEventHandlers.patchDescriptionContent(
        e.target.parentNode.parentNode.parentNode.id,
        descriptionContent,
      );
      e.target.value = '';
